...
; ** Supported trackers **
; - (old) DOC Soundtracker    9   (dst_timingcia=TRUE)
; - (old) Master Soundtracker 1.0 (dst_timingcia=FALSE)
...
; ** Play music trigger **
; - Label dst_PlayMusic: Called from CIA-B timer A or vblank interrupt
; - Equal dst_timingcia: TRUE = Activate sourcecode for CIA-B timer A as
...
dst_CIABCRABITS          EQU CIACRBF_LOAD ;CIA-B timer A continuous mode
dst_CIABCRBBITS          EQU CIACRBF_LOAD+CIACRBF_RUNMODE ;CIA-B timer B oneshot mode

; ** Timer values **
dst_120bpmtime           EQU 14640 ;= (240 - 120 BPM) * 122 (Karsten Obarski)
dst_dmawaittime          EQU 342 ;= 0.709379 MHz * [482.68 탎 = Lowest note period C1 * 2 / PAL clock constant = 856*2/3546895 ticks per second]
...
dst_minticks             EQU 1
dst_maxticks             EQU 15
dst_defaultbpm           EQU 120
dst_minbpm               EQU 0
dst_maxbpm               EQU 220
dst_bpmconst             EQU 240
dst_speedconst           EQU 122
...
dst_InitTimers
  IFEQ dst_timingcia
    moveq   #16,d0
    neg.b   d0                 ;240 = dst_bpmconst
    move.l  dst_SongDataPtr(a3),a0 ;Pointer to song data
    moveq   #TRUE,d1           ;NULL for word access
    move.b  dst_sd_songspeed(a0),d1 ;Get songsspeed in BPM
    sub.w   d1,d0              ;dst_bpmconst - songspeed
    mulu.w  #dst_speedconst,d0 ;(dst_bpmconst - songspeed) * 122
...
dst_InitSkip
  ...
  add.w   #dst_sd_sampleinfo+dst_si_samplelength,a0 ;First sample length
  lsl.w   #BYTESHIFTBITS,d1  ;*(dst_pattsize/8) = Offset points to end of last pattern
...
; ** Effect command 1xy "Normal play" or "Arpeggio" **
  CNOP 0,4
dst_Arpeggio
  move.w  dst_Counter(a3),d0 ;Get ticks
...
dst_GetNewNote
  ...
  move.b  (dst_sd_pattpos,a0,d0.w),d1 ;Get pattern number in song position table
...
; ** Get new note data **
  CNOP 0,4
dst_PlayVoice
  move.l  (dst_sd_patterndata,a0,d1.l*4),(a2) ;Get new note data from pattern
  ...
  movem.w dst_sd_sampleinfo+dst_si_samplelength(a0,d2.w*2),d0/d2-d4 ;length, volume, repeat point, repeat length
...
dst_CheckMoreEffects
  ...
  subq.b  #8,d0              ;0..8 ?
  ble.s   dst_ChkMoreEfxEnd  ;Yes -> skip
  subq.b  #4,d0              ;C "Set Volume" ?
  beq.s	  dst_SetVolume
  subq.b  #2,d0              ;E "Set Filter" ?
  beq.s	  dst_SetFilter
...
dst_SetFilter
  ...
  bne.s   dst_FilterOff      ;If 1-filter off -> skip
dst_FilterOn
...
dst_StopMusic
  ...
  lea     _CIAA-_CIAB(a5),a4 ;Base of CIA-A
  move.l  #_CUSTOM,a6        ;Base of custom chips
  bsr.s   dst_StopTimers
  bra.s   dst_SetBackRegisters
