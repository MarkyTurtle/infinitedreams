...
ost_InitSkip
  ...
  add.w   #ost_sd_sampleinfo+ost_si_samplelength,a0 ;First sample length
  lsl.w   #BYTESHIFTBITS,d1  ;*(ost_pattsize/8) = Offset points to end of last pattern
...
ost_GetNewNote
  ...
  move.b  (ost_sd_pattpos,a0,d0.w),d1 ;Get pattern number in song position table
...
; ** Get new note data **
  CNOP 0,4
ost_PlayVoice
  move.l  (ost_sd_patterndata,a0,d1.l*4),(a2) ;Get new note data from pattern
  ...
  movem.w ost_sd_sampleinfo+ost_si_samplelength(a0,d2.w*2),d0/d2-d4 ;length, volume, repeat point, repeat length
