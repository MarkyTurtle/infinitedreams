...
st_InitSkip
  ...
  add.w   #st_sd_sampleinfo+st_si_samplelength,a0 ;First sample length
  lsl.w   #BYTESHIFTBITS,d1  ;*(st_pattsize/8) = Offset points to end of last pattern
  moveq   #TRUE,d2           ;NULL to clear first word in sample data
  lea	  st_sd_patterndata-st_sd_id(a1,d1.w*8),a2 ;Skip MOD-ID and patterndata -> Pointer to first sample data in module
...
st_GetNewNote
  ...
  move.b  (st_sd_pattpos,a0,d0.w),d1 ;Get pattern number in song position table
...
st_PlayVoice
  ...
  move.l  (st_sd_patterndata,a0,d1.l*4),(a2) ;Get new note data from pattern
  ...
  movem.w st_sd_sampleinfo+st_si_samplelength(a0,d2.w*2),d0/d2-d4 ;length, volume, repeat point, repeat length
