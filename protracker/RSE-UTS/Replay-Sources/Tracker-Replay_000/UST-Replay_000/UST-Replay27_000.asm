...
ust_InitSkip
  ...
  lea     ust_sd_sampleinfo+ust_si_samplelength(a0),a0 ;First sample length
  swap    d1                 ;*ust_pattsize
...
ust_CheckEffects
  ...
  moveq   #ust_cmdnummask,d0 ;Get channel effect command without nibble of sample number
  and.b   n_cmd(a2),d0       ;Get effect command number
...
ust_GetNewNote
  ...
  add.w   #ust_sd_pattpos,d0 ;Offset pattern position table
  moveq	  #TRUE,d1           ;NULL needed for word access
  move.b  (a0,d0.w),d1       ;Get pattern number in song position table
  lsl.w   #BYTESHIFTBITS,d1  ;*(ust_pattsize/4) = Pattern offset
...
ust_PlayVoice
  ...
  move.l  ust_sd_patterndata-(127*4)(a0,d0.l),(a2) ;Get new note data from pattern
  ...
  movem.w ust_sd_sampleinfo+ust_si_samplelength(a0,d2.w),d0/d2-d4 ;length, volume, repeat point, repeat length
