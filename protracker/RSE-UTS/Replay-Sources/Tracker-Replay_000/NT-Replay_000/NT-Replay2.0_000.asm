...
nt_InitSkip
  ...
  lea     nt_sd_sampleinfo+nt_si_samplelength(a0),a0 ;First sample length
  swap    d1                 ;*nt_pattsize
  lsr.l   #6,d1              ;Offset points to end of last pattern
  moveq   #TRUE,d2           ;NULL to clear first word in sample data
  lea	  nt_sd_patterndata-nt_sd_id(a1,d1.l),a2 ;Skip MOD-ID and patterndata -> Pointer to first sample data in module
...
nt_CheckEffects
  ...
  moveq   #nt_cmdnummask,d0  ;Get channel effect command without lower nibble of sample number
...
nt_GetNewNote
  ...
  add.w   #nt_sd_pattpos,d0  ;Offset pattern position table
...
nt_PlayVoice
  ...
  move.l  nt_sd_patterndata-(255*4)(a0,d0.l),(a2) ;Get new note data from pattern
  ...
  movem.w nt_sd_sampleinfo+nt_si_samplelength(a0,d2.w),d0/d2-d4 ;length, volume, repeat point, repeat length
