...
; ** Additional features **
; - Metronom can be enabled for each audio channel
; - Audio channels can be seperately masked out
...
pt_metronoteinfo         EQU ((pt_metrosamplenum&NIBBLEMASKHI)*WORDSHIFT*BYTESHIFT)+(pt_metronote*WORDSHIFT)+((pt_metrosamplenum&NIBBLEMASKLO)*BYTESHIFT*NIBBLESHIFT)
...
; ** Allocate dummy word in CHIP memory **
  CNOP 0,4
pt_AllocDummyDataMem
  moveq   #2,d0              ;Size = 1 word
  move.l  #MEMF_CHIP+MEMF_CLEAR+MEMF_PUBLIC,d1 ;CHIP memory
  jsr     _LVOAllocMem(a6)   ;Allocate dummy word
  move.l  d0,pt_DummyData(a3) ;Save pointer to dummy word
  beq.s   pt_AllocDataMemError ;NULL !? -> skip
  moveq   #TRUE,d0           ;Returncode = OK
  rts
  CNOP 0,4
pt_AllocDataMemError
...
pt_InitVariables
  ...
  move.b  d0,pt_MetroChannel(a3) ;NULL = Disable metronome channel - Enable metronome channel with value 1,2,3 or 4
...
pt_InitSkip
  ...
  lea     pt_sd_sampleinfo+pt_si_samplelength(a0),a0 ;First sample length
  swap    d1                 ;*pt_pattsize
  lsr.l   #6,d1              ;Offset points to end of last pattern
  moveq   #TRUE,d2           ;NULL to clear first word in sample data
  lea	  pt_sd_patterndata-pt_sd_id(a1,d1.l),a2 ;Skip MOD-ID and patterndata -> Pointer to first sample data in module
...
pt_ChkEfx2
  ...
  beq.s	  pt_ChkEfx2End      ;If no command -> skip
  moveq   #pt_cmdnummask,d0  ;Get channel effect command number without lower nibble of sample number
...
pt_ExtCommands
  ...
  subq.b  #8,d0              ;0-7 ?
  blt.s   pt_ExtCommandsEnd
  beq	  pt_KarplusStrong   ;8 "Karplus Strong" ?
...
pt_GetNewNote
  ...
  add.w   #pt_sd_pattpos,d0  ;Offset pattern position table
...
pt_CheckMetronome
  ...
  beq.s	  pt_ChkMetroEnd     ;If NULL -> skip
...
pt_PlvSkip
  ...
  add.l   d0,d0              ;*4
  add.l   d0,d0
  move.l  pt_sd_patterndata-(255*4)(a0,d0.l),(a2) ;Get new note data from pattern
...
pt_FtuFound
  ...
  add.w   d0,d0               ;Offset in periods table*2
...
pt_StpGoss
  add.w   d0,d0              ;Counter*2
  move.w  (a1,d0.w),d3       ;Get table note period
pt_StpNoFinetune
...
; ** First note data in first pattern has a period but no sample number and sample start **
  CNOP 0,4
pt_NoSampleStart
  move.l  pt_DummyData(a3),d0 ;Pointer to to dummy word in CHIP memory
...
; ** Effect command E80 "Karplus Strong" **
  CNOP 0,4
pt_KarplusStrong
  move.w  n_replen(a2),d7    ;Get repeat length
  subq.w  #1,d7              ;-1 for dbf loop
  move.l  a0,-(a7)
  add.w   d7,d7              ;*2 = repeat length in bytes
...
pt_SetAllChanDMA
  ...
  and.w	  pt_ActiveChannels(a3),d0 ;Mask out inactive audio channels
...
pt_StopMusic
  ...
  move.l  #_CUSTOM,a6        ;Base of custom chips
  bsr.s   pt_StopTimers
  bsr.s   pt_SetBackRegisters
  move.l  ExecBase.w,a6      ;Base of exec library
  bra.s   pt_FreeDummyDataMem
...
; ** Free dummy data word in CHIP memory **
  CNOP 0,4
pt_FreeDummyDataMem
  move.l  pt_DummyData(a3),a1 ;Pointer to dummy word
  moveq   #2,d0              ;Size 1 word
  jmp     _LVOFreeMem(a6)    ;Free dummy word
